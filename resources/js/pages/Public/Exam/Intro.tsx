...
export default function Intro({ locale, attempt, exam, exam_type_name, applicant }: IntroProps) {
    ...
    return (
        <>
            <Head title={t('publicExam.intro.pageTitle', { name: exam.name })} />
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <ul className="space-y-3 rounded-xl bg-indigo-50 p-4 text-sm text-indigo-900">
                                <li className="flex items-center gap-3">
                                    ...
                                    <span>{t('publicExam.intro.questions', { count: exam.questions_count })}</span>
                                </li>
                                <li className="flex items-center gap-3">
                                    <Clock className="h-5 w-5 shrink-0" />
                                    <span>{t('publicExam.intro.duration', { count: exam.duration_minutes })}</span>
                                </li>
                            </ul>

                            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
                                {t('publicExam.intro.timerWarning')}
                            </div>

                            <form onSubmit={start}>
                                <Button type="submit" className="h-12 w-full text-base" size="lg">
                                    {t('publicExam.intro.startButton')}
                                </Button>
                            </form>
                        </CardContent>
                    </Card>
